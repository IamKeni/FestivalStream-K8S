# Installatiehandleiding - FestivalStream Kubernetes Cluster

Deze map bevat het volledige Ansible-project voor het opzetten van het Kubernetes-platform van FestivalStream. Het playbook installeert en configureert automatisch het gehele cluster: van de container runtime tot monitoring, logging, GitOps en de applicatie zelf.
...
| Worker 1 | k8s-worker1  | 192.168.1.51   | Ubuntu 22.04   | k8sworker1.leerteam3.lab |
| Worker 2 | k8s-worker2  | 192.168.1.52   | Ubuntu 22.04   | k8sworker2.leerteam3.lab |
...
- 3 Ubuntu 22.04 nodes bereikbaar via SSH (minimaal 2 vCPU's en 4 GB RAM per node)
...
Pas de volgende twee bestanden aan naar jouw omgeving voordat je het script uitvoert.
...
Dit playbook installeert automatisch een volledig Kubernetes-platform in 13 stappen. Alle stappen zijn idempotent: het script kan veilig opnieuw worden uitgevoerd zonder fouten.
...
| 7 | MetalLB LoadBalancer, IP-pool `192.168.1.60–69` | `metallb-system` | `metallb` |
| 8 | NGINX Ingress Controller, TLS-terminatie, routes `/web` + `/stream` | `ingress-nginx` | `ingress` |
| 9 | NFS CSI Driver (DaemonSet), StorageClass `nfs-client` (default) | `nfs-system` | `nfs` |
| 10 | Metrics Server + kube-prometheus-stack (Prometheus, Grafana, Alertmanager) | `monitoring` | `monitoring` |
| 11 | Loki + Promtail (DaemonSet), PVC `audit-logs-storage` (NFS), Loki datasource in Grafana, RBAC + NetworkPolicies | `logging` | `logging` |
| 12 | ArgoCD via officiële manifests, NodePort 32080/32443 | `argocd` | `argocd` |
| 13 | Web Interface + Streaming & VOD Service, PVCs, HPA, RBAC + NetworkPolicies | `applicatie` | `applicatie` |
...
| `metallb-system` | MetalLB speaker + controller |
...
| Rolling Update Strategy op beide Deployments | `applicatie` | Geen downtime bij updates (US-05.1) |
| Self-healing via liveness/readiness probes | `applicatie` | Automatisch herstellen bij crash (US-09.1/US-09.2) |
...
├── inventory.example      # Voorbeeldbestand (veilig om te committen)
...
│   └── applicatie/
│       ├── web/
│       │   └── deployment.yml     # Kubernetes manifest: Web Interface
│       └── stream/
│           └── deployment.yml     # Kubernetes manifest: Streaming & VOD Service
└── roles/
    ...
    ├── worker/            # cluster join
    ├── metallb/           # MetalLB LoadBalancer (namespace: metallb-system)
